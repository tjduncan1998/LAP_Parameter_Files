...
#            print(tau,tau_st)
            dz.append((avz,varz/tau*1e-7**2/1e-12,varz_block/integrate.trapezoid(ct,t_x)*1e-7**2/1e-12,varz))
    ...
    ax[1].errorbar(t_x,np.average(np.array(ct_collect),axis=0),yerr=np.std(np.array(ct_collect),axis=0),errorevery=100,label='%s cutoff' % cutoff); ax[1].legend(); ax[1].set_xlabel('Time (ps)'); ax[1].set_ylabel('S(t)')
    ax[0].set_ylabel('$D_{ion}$'); ax[0].set_xlabel('Distance from Channel Center (nm)')
    dz = np.array(dz)
    dz = dz[dz[:,0].argsort()]
...
def write_data(dz,header,fileout):
    c = open('{}_hist.dat'.format(fileout),'w')
    c.write(",".join(header)+'\n')
    for i in range(len(dz[:,0])):
        c.write("{}\n".format(' '.join(str(dz[i,1])[1:-1].split())))
        c.write("{}\n".format(' '.join(str(dz[i,0])[1:-1].split())))
       
    c.close()
    return

def hist_gen(t,pos,cutoff,step,sample,ax,zeros):
    hist=[]
    for i in range(len(zeros)-1):
        hist.append(np.histogram(pos[zeros[i]:zeros[i+1]],bins=10))
...
def main(tpr,trj,step,sample,sel1,sel2,cutoff,fileout):
    ...
    hist=hist_gen(t,pos,cutoff,step,sample,ax,zeros)
    print(hist)
#    if float(cutoff) == 0.0:
...
#plt.savefig('figures/frp_stl.pdf', format = 'pdf', dpi = 1200)
    ...
    write_data(hist,('z','Position','Histogram','alpha'),fileout)
